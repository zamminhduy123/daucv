...
import { Upload, FileText, X, CheckCircle, AlertTriangle, Sparkles, Mic, Loader2, Briefcase } from "lucide-react";
...
export default function SetupPage() {
  ...
  const [localCv, setLocalCv] = useState(cvText);
  const [localJd, setLocalJd] = useState(jdText);
  const [cvFile, setCvFile] = useState<File | null>(null);
  const [cvFileName, setCvFileName] = useState("CV của tôi");
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        setCvFile(null);
        setLocalCv(`[Lỗi trích xuất: ${result.error}]`);
      } else {
        setLocalCv(result.text || "");
      }
    } catch (err: any) {
    ...
    <div className="flex flex-col gap-4 h-[calc(100vh-160px)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 min-h-0">
        ...
        <TextCard
          ...
          wordCountText={wordCount(localJd)}
        >
          <textarea
            value={localJd}
        ...
        <TextCard
          ...
          wordCountText={wordCount(localCv)}
          headerRight={
            <button
            ...
            cvFile ? (
              ...
              >
                <span className="flex items-center gap-1.5 text-xs font-semibold text-[#2F4F4F] truncate">
                  ...
                  {isExtractingPDF ? "Đang đọc PDF..." : cvFile.name}
                </span>
