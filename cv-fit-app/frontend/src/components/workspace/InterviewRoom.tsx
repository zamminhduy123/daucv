...
export default function InterviewRoom({ cvText, jdText, initialState, totalQuestions, interviewType, onBack }: InterviewRoomProps) {
  ...
  const { voiceEnabled, setVoiceEnabled, speak, isSpeaking, isLoading: isLoadingTTS } = useTTS();
  ...
  return (
    <div className="max-h-[calc(100vh-88px)] py-1 md:py-2 px-1 w-full bg-[#F9F9F2] text-[#2F4F4F] font-sans overflow-hidden flex flex-col gap-3">
      ...
      {/* 1. MAIN CENTER AREA: Conversation (75%) */}
        <main className="flex-1 flex flex-col bg-white rounded-2xl md:rounded-3xl shadow-sm border border-[#2F4F4F]/5 relative overflow-hidden">
      ...
      {/* 2. RIGHT SIDEBAR: Live Metrics & Hints (Mostly hidden on mobile, or bottom scroll) */}
        <aside className="hidden lg:flex w-75 xl:w-85 min-h-0 flex-col bg-white rounded-3xl p-4 shadow-sm border border-[#2F4F4F]/5 overflow-hidden">
